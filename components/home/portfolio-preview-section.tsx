...
const sizeConfig: Record<string, { cols: string; rows: string }> = {
  large: { cols: "md:col-span-2", rows: "md:row-span-2" },
  medium: { cols: "md:col-span-1", rows: "md:row-span-1" },
  small: { cols: "md:col-span-1", rows: "md:row-span-1" },
}
...
export function PortfolioPreviewSection() {
  ...
  useRealtimeUpdates("portfolio", (payload) => {
    console.log("[v0] Portfolio updated:", payload)
    fetchPortfolio()
  })
  ...
  async function fetchPortfolio() {
    try {
      ...
      let query = supabase
        .from("portfolio")
        .select("id, title, category, featured_image, slug, is_featured")
        .eq("status", "published")
  ...
  const getItemSize = (item: PortfolioItem, index: number) => {
    if (item.is_featured && index === 0) return "large"
    return "small"
  }
  ...
  return (
    <section className="py-20 lg:py-28 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                selectedCategory === category
                  ? "bg-[#C4D600] text-[#1C1C1C]"
                  : "bg-white text-[#1C1C1C] border border-gray-300 hover:border-[#C4D600]"
              }`}
            >
              {category}
            </button>
        ...
        {/* Portfolio Grid - Bento Style with Dynamic Sizing */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {portfolioItems.map((item, idx) => {
            const size = getItemSize(item, idx)
            ...
            return (
              <Link
                ...
                className={`group relative overflow-hidden rounded-2xl bg-[#1C1C1C] ${sizeClasses.cols} ${sizeClasses.rows}`}
              >
                <div className={`relative ${size === "large" ? "h-[400px] md:h-full" : "h-64"}`}>
                  <Image
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <h3 className={`font-bold text-white ${size === "large" ? "text-2xl" : "text-lg"}`}>
                      {item.title}
                    </h3>
                  </div>
